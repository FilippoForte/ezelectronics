      ...
      try {
        ...
        db.get(sql1, [username], (err: Error, row: any) => {
          ...
          if (!row) {
            ...
            (username != user.username && user.role != "Admin") ||
            (user.role == "Admin" &&
              row.role == "Admin" &&
  ...
  async deleteUser(user: User, username: string): Promise<boolean> {
    return new Promise<boolean>((resolve, reject) => {
      try {
        ...
        db.get(selectQuery, [username], (err: Error | null, row: any) => {
          if (err) {
            return reject(err);
          }
          if (!row) {
          ...
          const userToDelete: User = new User(
            row.username,
            row.name,
            row.surname,
            row.role,
          ...
          if (
            ...
            db.run(deleteQuery, [username], (err: Error) => {
              if (err) {
                return reject(err);
              } else {
                return resolve(true);
